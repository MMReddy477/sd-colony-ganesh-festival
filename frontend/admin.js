...
function renderDonationTable(items, path) {
  ...
  const filtered = items.filter(item => `${item.flatNumber} ${item.donorName} ${item.mobile} ${item.amount} ${item.paymentMode} ${item.status} ${item.contributionType} ${item.itemName}`.toLowerCase().includes(query) && (!mode || item.paymentMode === mode)).sort(sortByPlotNumber);
