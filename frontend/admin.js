...
const donationPopupObserver = new MutationObserver(async () => {
  ...
  modal.querySelector(".finance-content").innerHTML =
    `<div class="donation-popup-wrap"><table class="donation-popup-table"><thead><tr><th>Flat Number</th><th>Donor Name</th><th>Mobile Number</th><th>Amount</th><th>Payment Mode</th><th>Date</th><th>Receipt</th></tr></thead><tbody>${data.donations.map((item) => `<tr><td>${escapeHtml(item.flatNumber || "--")}</td><td>${escapeHtml(item.donorName || "--")}</td><td>${escapeHtml(item.mobile || "--")}</td><td><strong>${money(item.amount)}</strong></td><td>${escapeHtml(item.paymentMode || "--")}</td><td class="date-nowrap">${formatDate(item.date || item.createdAt)}</td><td>${item.receiptNumber ? `<button class="receipt-action" type="button" data-view-receipt="${escapeHtml(item.receiptNumber)}">👁 View Receipt</button> <button class="receipt-action" type="button" data-download-receipt="${escapeHtml(item.receiptNumber)}">⬇ Download Receipt</button>` : "--"}</td></tr>`).join("") || '<tr><td colspan="7">No donations recorded yet.</td></tr>'}</tbody></table></div>`;
});
