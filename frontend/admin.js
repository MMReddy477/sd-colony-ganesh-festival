...
const donationPopupObserver = new MutationObserver(async () => {
  ...
  const renderDonationPopup = () => {
    ...
    content.innerHTML = `<div class="admin-finance-tools"><label for="adminFinanceDonationSearch">Search donor name<input id="adminFinanceDonationSearch" data-admin-donation-search type="search" placeholder="Search by name" value="${escapeHtml(query)}"></label><span>Showing ${first}-${last} of ${filtered.length} donations</span></div><div class="donation-popup-wrap admin-finance-donation-wrap"><table class="donation-popup-table admin-finance-donation-table"><colgroup><col class="col-flat"><col class="col-name"><col class="col-mobile"><col class="col-amount"><col class="col-mode"><col class="col-actions"></colgroup><thead><tr><th>Plot No.</th><th>Donor Name</th><th>Mobile Number</th><th>Amount</th><th>Payment Mode</th><th>Actions</th></tr></thead><tbody>${visible.map((item) => `<tr><td data-label="Plot No.">${escapeHtml(normalizePlotNumber(item.flatNumber) || "--")}</td><td data-label="Donor Name">${escapeHtml(item.donorName || "--")}</td><td data-label="Mobile Number">${escapeHtml(item.mobile || "--")}</td><td data-label="Amount" class="amount-nowrap"><strong>${money(item.amount)}</strong></td><td data-label="Payment Mode">${escapeHtml(item.paymentMode || "--")}</td><td data-label="Actions" class="actions-cell"><div class="admin-actions"><button class="admin-icon-btn" type="button" data-edit-donation="${item._id}" title="Edit donation" aria-label="Edit donation">✎</button>${item.receiptNumber ? `<button class="receipt-action" type="button" data-view-receipt="${escapeHtml(item.receiptNumber)}" title="View Receipt" aria-label="View Receipt">👁</button><button class="receipt-action" type="button" data-download-receipt="${escapeHtml(item.receiptNumber)}" title="Download Receipt" aria-label="Download Receipt">⬇</button>` : ""}</div></td></tr>`).join("") || '<tr><td colspan="6" class="donor-empty">No donations found.</td></tr>'}</tbody></table></div><div class="admin-finance-pagination"><button type="button" data-admin-donation-page="prev" ${page === 0 ? "disabled" : ""}>Previous</button><span>Page ${page + 1} of ${pages}</span><button type="button" data-admin-donation-page="next" ${page >= pages - 1 ? "disabled" : ""}>Next</button></div>`;
...
function renderDonationTable(items, path) {
  ...
  const filtered = items.filter(item => `${item.flatNumber} ${item.donorName} ${item.mobile} ${item.amount} ${item.paymentMode}`.toLowerCase().includes(query) && (!mode || item.paymentMode === mode)).sort(sortByPlotNumber);
