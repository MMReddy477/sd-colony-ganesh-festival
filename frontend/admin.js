...
function renderExpenseTable(items, path) {
  ...
  container.innerHTML = `<div class="expense-table-wrap"><table class="expense-table"><thead><tr><th>Expense name</th><th>Total</th><th>Advance</th><th>Remaining</th><th>Status</th><th>Payment mode</th><th>Date</th><th>Time</th><th>Actions</th></tr></thead><tbody>${visible.map((item) => `<tr><td data-label="Expense name" class="expense-name"><span class="expense-value">${item.name || "--"}</span></td><td data-label="Total" class="expense-total"><strong class="expense-value">${money(item.amount)}</strong></td><td data-label="Advance / Paid" class="expense-paid"><span class="expense-value">${money(item.advanceAmount || 0)}</span></td><td data-label="Remaining / Due" class="expense-due"><span class="expense-value">${money(item.remainingAmount || 0)}</span></td><td data-label="Status" class="expense-status expense-status-${(item.status || "Due").toLowerCase().replace(/\s+/g, "-")}"><span class="expense-value">${item.status || "Due"}</span></td><td data-label="Payment mode"><span class="expense-value">${item.paymentMode || "--"}</span></td><td data-label="Date"><span class="expense-value">${formatExpenseDate(item)}</span></td><td data-label="Time"><span class="expense-value">${formatExpenseTime(item)}</span></td><td data-label="Actions" class="actions-cell"><div class="admin-actions donation-actions"><button class="admin-icon-btn edit-action" type="button" data-edit-expense="${item._id}" title="Edit expense" aria-label="Edit expense">✎</button><button class="admin-icon-btn delete-btn" type="button" data-delete="${path}/${item._id}" title="Delete expense" aria-label="Delete expense">🗑</button></div></td></tr>`).join("") || '<tr><td colspan="9" class="muted">Nothing here yet.</td></tr>'}</tbody></table></div>`;
...
function renderDonationTable(items, path) {
  ...
  const familyPlotPattern = /^(sirius|samyukta)(?:[-_ ]?\d+)?$|^plot\s*no[- ]?04$|^plant\s*no[- ]?35\/p$/i;
  const scopedItems = document.body.classList.contains("family-finance-only")
    ? items.filter(item => familyPlotPattern.test(String(item.flatNumber || "").trim()))
    : items;
  ...
  const filtered = scopedItems.filter(item => { const type = item.contributionType || "Regular Donation"; const matchesContribution = contribution === "" || (contribution === "regular" ? !["Laddu Auction 2025", "Ganesh Idol Sponsor", "Laddu Sponsorship 2026"].includes(type) : type === contribution); return `${item.flatNumber} ${item.donorName} ${item.mobile} ${item.amount} ${item.paymentMode} ${item.status} ${type} ${item.itemName}`.toLowerCase().includes(query) && (!mode || item.paymentMode === mode) && (!status || (item.status || "Received") === status) && matchesContribution; }).sort(sortByPlotNumber);
