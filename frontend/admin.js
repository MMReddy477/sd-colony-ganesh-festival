...
const formatExpenseDateTime = item => {
  ...
  const formattedDate = Number.isNaN(parsedDate.getTime()) ? formatDate(date) : `${String(parsedDate.getDate()).padStart(2, "0")}-${month}-${parsedDate.getFullYear()}`;
  return `${formattedDate} ${new Date(time).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}`;
};
