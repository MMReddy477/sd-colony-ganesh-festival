...
document.addEventListener("click", async (event) => {
  ...
  if (contributionType) {
    ...
    const rows = donations.map(item => `<tr><td>${escapeHtml(normalizePlotNumber(item.flatNumber) || "--")}</td><td>${escapeHtml(item.donorName || "--")}</td>${isAuction ? `<td>${escapeHtml(item.itemName || "--")}</td>` : ""}<td class="amount-positive">${money(item.amount)}</td><td>${formatDate(item.date || item.createdAt)}</td><td>${escapeHtml(item.paymentMode || "Cash")}</td><td>${item.receiptNumber ? `<span class="public-receipt-actions"><button class="receipt-action" type="button" data-public-receipt="${escapeHtml(item.receiptNumber)}" title="View receipt" aria-label="View receipt">&#128065;</button><button class="receipt-action public-receipt-row-download" type="button" data-public-download="${escapeHtml(item.receiptNumber)}" title="Download receipt" aria-label="Download receipt">&#11123;</button></span>` : "--"}</td></tr>`).join("");
    const columns = isAuction ? "<th>Plot No.</th><th>Donor Name</th><th>Item Name</th><th>Amount</th><th>Date</th><th>Payment Mode</th><th>Actions</th>" : "<th>Plot No.</th><th>Donor Name</th><th>Amount</th><th>Date</th><th>Payment Mode</th><th>Actions</th>";
