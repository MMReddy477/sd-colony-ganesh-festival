...
async function loadPortal({ refresh = false } = {}) {
  ...
  if (selectedFinanceView === "family") {
    ...
    if (donationsHeading) donationsHeading.textContent = "Party Contributions";
  }
  ...
  document.getElementById("eventsList").innerHTML = publicEvents.length
    ? `<div class="member-table-wrap public-events-table-wrap"><table class="member-table event-table public-events-table"><thead><tr><th>Devotee Name</th><th>Contact Number</th><th>Pooja Type</th><th>Preferred Date</th><th>Preferred Time</th><th>Venue / Location</th><th>Request Status</th><th>Actions</th></tr></thead><tbody>${visibleEvents.map(e => { const status = e.status || "Pending"; return `<tr><td data-label="Devotee Name"><strong>${e.name || "--"}</strong></td><td data-label="Contact Number">${e.mobile || "--"}</td><td data-label="Pooja Type">Ganesh Pooja</td><td data-label="Preferred Date">${e.date ? date(e.date) : "--"}</td><td data-label="Preferred Time">${e.time || "--"}</td><td data-label="Venue / Location">${e.venue || "Between Sirius & Samyukta"}</td><td data-label="Request Status"><span class="ritual-status ritual-status-${status.toLowerCase()}">${status}</span></td><td data-label="Actions"><span class="ritual-public-action" aria-label="Pooja request details">👁</span></td></tr>`; }).join("")}</tbody></table></div>`
...
function renderPublicDonors(items) {
  ...
  publicDonorRows = (selectedFinanceView === "family" ? items.filter(item => familyPlotPattern.test(String(item.flatNumber || "").trim())) : [...items]).sort(sortByPlotNumber);
  ...
  const filtered = publicDonorRows.filter(item => `${item.flatNumber} ${item.donorName}`.toLowerCase().includes(query) && (!mode || item.paymentMode === mode));
